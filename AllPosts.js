...
function displayPosts(posts) {
  ...
  const limitedPosts = posts.slice(start, end);
  ...
  nextButton.disabled = end >= posts.length; // Disable next button on last page
}
...
document.getElementById("dateInput").addEventListener("change", filterPosts);
document.getElementById("yearInput").addEventListener("input", filterPosts);
document.getElementById("monthInput").addEventListener("change", filterPosts);
